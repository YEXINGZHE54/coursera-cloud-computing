...
public class RankCarrierByArr extends Mapper<LongWritable, Text,Text,DoubleWritable> {
    public static final Log log = LogFactory.getLog(RankCarrierByArr.class);
    ...
    */
    public void map(LongWritable key, Text value, Mapper.Context context) throws IOException, InterruptedException {
        ...
        if (!fields[3].equals(dest)) {
            return; //not arrive at Y, just skip
        }
        word.set(fields[1]);
    ...
    public static void main(String[] args) throws Exception {
        ...
        conf.set(DestKey, args[3]);
        FileSystem fs = FileSystem.get(conf);
        Job job = Job.getInstance(conf);
        job.setJarByClass(RankCarrierByArr.class);
        job.setMapperClass(RankCarrierByArr.class);
